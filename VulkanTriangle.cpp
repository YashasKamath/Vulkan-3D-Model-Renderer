...
void VulkanTriangle::createLogicalDevice() {
	...
	deviceFeatures.samplerAnisotropy = VK_TRUE; // need to specifically enable the anisotropy feature if we are using it later on.
...
void VulkanTriangle::createDescriptorSetLayout() {
	...
	uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; // shader stages in which the descriptor is referenced.
	uboLayoutBinding.pImmutableSamplers = nullptr; // used only for image sampling related descriptors
...
void VulkanTriangle::createTextureImage() {
	...
	// The idea here is to load the image into a pixels array and then place it into the staging buffer, from where it will then be copied
	// to the final image object for the shader to use. We can create staging image too here, but transfer of pixel data from buffer to 
	// image object is much faster in certain GPUs.
	...
	// CPU can't write data directly into GPU side memory, so it has to map a pointer of RAM to GPU side memory and then write to the RAM.
	// That RAM data will be synced or copied into the mapped GPU side memory automatically as COHERENT bit is enabled.
	void* data;
	...
	// Once the image is created with pixel data loaded into it, it needs to be read easily by the shader
...
void VulkanTriangle::createTextureSampler() {
	...
	samplerInfo.magFilter = VK_FILTER_LINEAR; // Linear interpolation for oversampling problem (fragments > texels)
	samplerInfo.minFilter = VK_FILTER_LINEAR; // Linear interpolation for undersampling problem (fragments < texels)
	// Tell whether the image has to be repeated, mirror repeated, clamped to (border / edge) etc when sampling goes beyond the texture image
	// dimensions
	...
	// Anisotropic filtering is technique of making the image look sharper and clear when texture is viewed at sharp angles
	samplerInfo.anisotropyEnable = VK_TRUE;
	samplerInfo.maxAnisotropy = properties.limits.maxSamplerAnisotropy;

	samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK; // which color to show up when the texture is clamped to border for sampling
	samplerInfo.unnormalizedCoordinates = VK_FALSE; // meaning, the texel coordinates will be within 0 and 1; popularly used option
	// else it will be within texWidth and texHeight which doesn't work for textures of varying resolutions.

	// Used in shadow maps for percentage-close filtering
...
void VulkanTriangle::createUniformBuffers() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		...
		// this mapping is called persistent mapping, as you map only once, and use it for the application's lifetime. Mapping is expensive op.
...
void VulkanTriangle::createDescriptorPool() {
	...
	poolInfo.maxSets = static_cast<uint32_t> (MAX_FRAMES_IN_FLIGHT); // specifying max number of descriptor sets that can be allocated.
...
void VulkanTriangle::createDescriptorSets() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		...
		descriptorWrites[0].dstArrayElement = 0; // descriptor set can be array also, and here, it isn't an array, so 0.
...
void VulkanTriangle::updateUniformBuffer(uint32_t currentImage) {
	...
	// identity matrix, rotation degree in radians, axis (Z) along which the rotation should happen
	ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	// camera: 45 degree from the top; origin; up vector (Z axis)
	ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ubo.proj = glm::perspective(glm::radians(45.0f), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);

	// GLM designed originally for OpenGl where Y-coord of clip coordinates are inverted. In Vulkan, we compensate that by this op.
	// Without this, the model rendered will be upside down.
	ubo.proj[1][1] *= -1;
...
void VulkanTriangle::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, 
	...
	imageInfo.format = format; // S in SRGB stands for standard
	// This defines how the texels are laid out in memory
	// VK_IMAGE_TILING_LINEAR - we might go to memory to access the texels of the image, not needed here as we used staging buffer
	// Hence, we used Optimal mode of tiling of texels.
	imageInfo.tiling = tiling;
	imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // content is garbage
	// We want the image to be used by the shader for coloring our mesh, so that's why sampled bit is enabled.
	imageInfo.usage = usage;
	imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // used only by graphics queue
	imageInfo.samples = VK_SAMPLE_COUNT_1_BIT; // No multisampling anti aliasing here.
...
void VulkanTriangle::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
	...
	// Need to specify which part of the buffer is copied to which part of the image.
...
void VulkanTriangle::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
	...
	// These are not default values, need to be explicitly specified if we don't want to transfer queue family ownership
